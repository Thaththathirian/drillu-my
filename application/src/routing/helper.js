...
export const convertToRoutes = ({
  ...
  return () => {
    const itemMapper = (item) => {
      ...
      // Ensure all paths include the collegeId parameter
      if (tempItem.path && !tempItem.path.includes(':collegeId')) {
        tempItem.path = `/:collegeId${tempItem.path}`;
      }
      if (tempItem.to && typeof tempItem.to === 'object' && tempItem.to.pathname && !tempItem.to.pathname.includes(':collegeId')) {
        tempItem.to.pathname = `/:collegeId${tempItem.to.pathname}`;
      }
